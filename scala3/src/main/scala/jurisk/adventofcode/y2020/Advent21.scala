...
object Advent21 extends SingleLineAdventApp[Entry, Int, String]:
  ...
  private def findCanonical(rules: Rules): Map[Allergen, Ingredient] =
    val simplified: Map[Allergen, Set[Ingredient]] =
      rules.allergens.map { allergen =>
          ...
          case Some((allergen, ingredientSet)) =>
            ingredientSet.toList match
              case ingredient :: Nil =>
                ...
                val newRemaining = (remaining - allergen).view
                  .mapValues(x => x - ingredient)
                  .toMap
  ...
  private def findSafe(
    rules: Rules,
    canonical: Map[Allergen, Ingredient],
  ): Set[Ingredient] =
