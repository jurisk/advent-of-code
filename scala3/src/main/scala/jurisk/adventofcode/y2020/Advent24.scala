...
object Advent24 extends SingleLineAdventApp[Flip, Int, Int]:
  ...
  final case class Field(black: Set[Coords] = Set.empty):
    private def interestingTiles: Set[Coords] =
      black ++ black.flatMap(_.neighbours)
    ...
    private def applyFlip(flip: Flip): Field =
      val coords =
        flip.moves.foldLeft(Coords(0, 0))((acc, move) => acc.applyMove(move))
      if black.contains(coords) then Field(black - coords)
      else Field(black + coords)
    ...
    private def willSurvive(c: Coords): Boolean =
      ...
      (weAreBlack, neighbouringBlacks) match
        case (true, n) if n == 0 || n > 2 =>
          false // Any black tile with zero or more than 2 black tiles immediately adjacent to it is flipped to white.
        case (false, 2)                   =>
          true // Any white tile with exactly 2 black tiles immediately adjacent to it is flipped to black.
        case (x, _)                       => x

    private def nextDay: Field = Field(interestingTiles.filter(willSurvive))
    def days(n: Int): Field    =
      (0 until n).foldLeft(this)((acc, _) => acc.nextDay)
