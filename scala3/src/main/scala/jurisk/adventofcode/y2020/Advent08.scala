...
import jurisk.adventofcode.y2020.Advent08.Instruction
...
object Advent08 extends SingleLineAdventApp[Instruction, Int, Int]:
  ...
  case class Program(
    ...
    def replaceInstruction(
      ...
      if indices.contains(instructionIdx) then
        copy(instructions =
          instructions.updated(instructionIdx, instruction)
        ).some
      else none

  object Program:
    def apply(instructions: Seq[Instruction]): Program = Program(
  ...
  case class Execution(
    ...
    @tailrec
    final def run(
      alreadyVisited: Set[InstructionIdx] = Set.empty
    ): ExecutionResult =
  ...
  def solution1(testCases: List[Instruction]): Int =
    ...
    Execution(program).run() match {
      ...
      case ExecutionResult.FinishedSuccessfully(accumulator) =>
        sys.error(s"Expected to halt in $program")
    }
