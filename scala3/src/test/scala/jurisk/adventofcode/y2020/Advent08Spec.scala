...
class Advent08Spec extends AnyFreeSpec:
  ...
  private def parse(x: String) = Advent08.parseTestCases(x.split(Properties.lineSeparator).filter(_.nonEmpty).toList)
  ...
  "solution2" in {
    ...
    Execution(program2).run() shouldEqual ExecutionResult.FinishedSuccessfully(8)
