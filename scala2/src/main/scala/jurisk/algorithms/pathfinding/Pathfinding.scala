...
object Pathfinding {
  /**
    ...
    * @param successors List of successors for a given node, along with the cost of moving from the given node to this successor.
    * @param heuristic Approximation of the cost from the given node to the goal. Must not be greater than the real cost, or a wrong path may be returned!
    * @param success Is the node reached? A function, not a node, because some problems require a dynamic solution.
    * @tparam N Node type
    * @tparam C Cost type
    * @return The shortest path starting from `start` up to a node for which `success` returns `true`, along with the total cost, in a `Some`. If no path can be found, `None` is returned instead.
    */
  def aStar[N, C : Numeric : Ordering : Bounded](
    ...
    val gScore: mutable.Map[N, C] = mutable.Map(start -> Zero).withDefaultValue(MaxValue)

    // For node n, fScore[n] := gScore[n] + h(n). fScore[n] represents our current best guess as to
    // how short a path from start to finish can be if it goes through n.
    val fScore: mutable.Map[N, C] = mutable.Map(start -> heuristic(start)).withDefaultValue(MaxValue)
  ...
  def dikstraUsingAStar[N, C: Numeric : Ordering : Bounded](
    start: N,
  ...
  def shortestPath[N](
    ...
    dikstraUsingAStar[N, Int](
      start,
      n => successors(n).map { x => x -> 1 },
      isGoal,
    ).map { case (path, cost) =>
      require(path.length - 1 == cost, "Expected path to match cost but it didn't")
      path
    }
  }
