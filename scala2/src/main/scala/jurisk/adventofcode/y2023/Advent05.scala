...
object Advent05 {
  ...
  val Order = Seed :: Soil :: Fertilizer :: Water :: Light :: Temperature :: Humidity :: Location :: Nil
  ...
  final case class Input(
    ...
    def seedsPart2: List[Long] = {
      assert(seedInput.length % 2 == 0)
      seedInput.grouped(2).flatMap { x =>
        val List(from, len) = x
        from until (from + len)
      }.toList
    }
    ...
    def seedToLocation(seed: Long): Long = {
      ...
      pairs foreach { case (from, to) =>
        val newCurrent = convert(current, from, to)
//        prLongln(s"Converting $current using $from to $to: got $newCurrent")
        current = newCurrent
      }
    ...
    def convert(n: Long, from: String, to: String): Long = {
      val conversionMap = conversionMaps.filter(m => m.from == from && m.to == to).head
  ...
  def part2(data: Input): Long =
    data.seedsPart2.map(x => data.seedToLocation(x)).min
